...
if(room==rm_battlescreen) {
	...
	var spr=attack_sprite, xo=sprite_get_xoffset(spr),yo=sprite_get_yoffset(spr),col=0,
		wid=sprite_get_width(spr),hgt=sprite_get_height(spr),xx=0,yy=0;
	...
	if(sprite_change) {
		...
		repeat(wid*hgt) {
			col=collision_rectangle((game_width/576)*xx,(game_height/324)*yy,
				(game_width/576)*xx,(game_height/324)*yy,obj_attrange,true,true);
			if(col==-4) col=1;
			else col=0;
			colarray[xx,yy]=col;
			xx++;
	...
	repeat(112) {
		...
		if(!flipstart) {
			...
			if(fliptrue[num-1]==1 && flipping[num-1]==1) {
				if(fliparray[num-1]<7) fliparray[num-1]+=flipspd;
				else {
					fliptrue[num-1]=0; 
		...
		if(fliptrue[num-1]==0 && num!=1 && fliparray[num-1]>0 && flipstart) {
			if(ix!=0 && fliparray[flip2D[ix-1,iy]-1]<=7 && iy!=0 && fliparray[flip2D[ix,iy-1]-1]<=7)
				fliparray[num-1]-=flipspd;
			else if((ix!=0 && fliparray[flip2D[ix-1,iy]-1]<=7) || (iy!=0 && fliparray[flip2D[ix,iy-1]-1]<=7)) 
				fliparray[num-1]-=flipspd;
		...
		var battlex=40+(ix*16),battley=26+(iy*16);

...
var tp=text[page],text_array_len=array_length(tp);
	...
	if(type[page] == 0) {
		...
		if(charCount<str_len && !textpause) {
			...
			switch(string_char_at(text_NE,floor(charCount))) {
				...
				play_talking(char,glopitch); break; //pitch[page]
			}
		}
			...
			repeat(charCount) {
				...
				switch(char) {
					...
					case "T":
					case "N":
					case "M": cx+=charSize*sc+6*sc; break;
					// 5-wide
					case "z":
					case "y":
					case "x":
					...
					case "L":
					case "I":
					case "J": cx+=charSize*sc+2*sc; break;
					// 4-wide
					case "k":
					case "j":
					case "(":
					...
					case "l":
					case "i":
					case "1": cx+=charSize*sc-2*sc; break;
					// 2-wide
					case ",":
					case "'":
					case ";": cx+=charSize*sc-4*sc; break;
